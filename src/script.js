...
const tick = () => {
  ...
  const aDrag = xDelta.clone().multiplyScalar(parameters.moveSpeed);
  // Acceleration from the damping force
  const aDamp = velocity.clone().multiplyScalar(parameters.damping);

  // Change the velocity
  velocity = velocity.add(aDrag).sub(aDamp);
